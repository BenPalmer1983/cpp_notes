...
// Group all the Maths classes/structs together.
namespace Maths
{
...
struct Vec3
{
    ...
    // Attributes - accessible without setters/getters
    double x {};
    double y {}; 
    double z {};
    ...
    // Output stream      std::cout << myvec << std::endl;
    ...
    friend inline Vec3 operator/(const Vec3& a, double s) {
        if(s == 0.0) throw std::invalid_argument("Divide by zero error.");
    ...
    {
        if(scalar == 0.0) throw std::invalid_argument("Divide by zero error.");
