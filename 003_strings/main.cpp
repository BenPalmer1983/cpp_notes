...
int main()
{
    ...
    // 5) String methods
    std::cout << "Append, find, substr." << std::endl;
    std::string sentence = "C++ strings are powerful.";
    sentence.append(" Indeed!");
    std::cout << "after append: " << sentence << std::endl;

    auto pos_1 = sentence.find("strings");
    std::cout << "find 'strings': position " << pos_1 << std::endl;

    auto pos_2 = sentence.find("Strings");
    std::cout << "find 'Strings': position " << pos_2 << std::endl;
    std::cout << "End position: " << std::string::npos << std::endl;
    ...
    std::string_view sv = modern_str;
    std::cout << "string_view: " << sv << " (size = " << sv.size() << ")\n" << std::endl;

    // Notice: sv does not own the memory. If modern_str is destroyed,
    // sv becomes invalid!
